...
public class ComputeSimilarity {
    ...
    public static void computeSimilarity(final SearchResult row,
                                         final String query,
                                         final Input input) {
        // keep max similarity
        row.setSimilarity(0);
    ...
    private static int computeEditDistance(final String s0, final String s1) {
        ...
        for (var j = 1; j < len1; j++) {
            ...
            for (var i = 1; i < len0; i++) {
                ...
                final var costDelete = newCost[i - 1] + 1; // deletion
                final var costInsert = cost[i] + 1; // insertion
                final var costReplace = cost[i - 1] + match; // substitution
                ...
                if (i > 1 && j > 1 && s0.charAt(i - 1) == s1.charAt(j - 2) && s0.charAt(i - 1) == s1.charAt(j - 1)) {
                    final var costTransposition = oldCost[i - 2] + match; // transposition
